...
void HandTest_ContainsCard()
{
	...
	assert (Hand_ContainsCard(h, Card_Make(DIAMOND, 5)));
	assert (!Hand_ContainsCard(h, Card_Make(SPADE, 1)));
}
...
void HandTest_RemoveCard()
{
	...
	Hand_RemoveCard(&h, Card_Make(CLUB, 2));
	assert (h.length == 2);
	Hand_RemoveCard(&h, Card_Make(DIAMOND, 5));	
	assert (h.length == 1);
	Hand_RemoveCard(&h, Card_Make(HEART, 1));
	assert (h.length == 0);
...
void HandTest_Sort()
{
	...
	assert ((h.cards[0].suit == HEART) && (h.cards[0].rank == 1));
	assert ((h.cards[1].suit == CLUB) && (h.cards[1].rank == 2));
	assert ((h.cards[2].suit == HEART) && (h.cards[2].rank == 3));
	assert ((h.cards[3].suit == DIAMOND) && (h.cards[3].rank == 5));
	assert ((h.cards[4].suit == SPADE) && (h.cards[4].rank == 8));
