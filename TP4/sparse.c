...
void SparseMatrix_AddValue(SparseMatrix *self, int row, int column, double element)
{
  ...
  if (element == 0 && !replacingValue)
  { // ? We want to replace a 0 with a 0, no change
    return;
  }
  if (element != 0 && replacingValue)
  { // ? We want to replace a value with another value, no alloc change
    self->elements[index] = element;
    return;
  }

  if (element == 0)
  { // ? Case we want to put a 0 on an existing value
    self->nbElements--;
  ...
  { // ? Case we want to put a value on an existing 0
    self->nbElements++;
    self->irows = realloc(self->irows, self->nbElements);
    self->icolumns = realloc(self->icolumns, self->nbElements);
    self->elements = realloc(self->elements, self->nbElements);
